"""
Compute the forward modeling in a 2D domain using Devito
...
def main(args):
    ...
    plt.savefig(os.path.join("data", "shots", 
                             f"{os.path.splitext(os.path.basename(vp_model_path))[0]}_aperture6000.png"),
                bbox_inches='tight')
    plt.show()

    # RICORDA DI CAMBIARE NOME PER IL SALVATAGGIO SE MODIFICHI APERTURE
...
if __name__ == "__main__":
    ...
    parser.add_argument('--vp_model_path', type=str, default="./data/v_models/marmousi_sp25.npz", help='Path to the true velocity model')
    ...
    parser.add_argument('--aperture', type=float, default=5000, help='Aperture in meters to the left and the right of the source in meters (default: np.inf takes the entire model)')
