...
def main(args):
    ...
    # AGGIUNTO IO
    # Select central shot
    n_shots = d_obs_list.shape[0]
    central_shot_idx = (n_shots) // 2
    central_shot = d_obs_list[central_shot_idx]
    ...
    # AGGIUNTO IO
    # Plot and save central shot image
    # Plot central shot in seismic style (grayscale)
    nx = central_shot.shape[1]
    nt = central_shot.shape[0]
    t_max_ms = nt * dt

    vmax = np.percentile(np.abs(central_shot), 99)
    ...
    plt.imshow(
        ...
        vmax=vmax,
        aspect='auto',
        extent=[0, nx, t_max_ms, 0]
    )

    plt.xlabel('Indice ricevitore')
    plt.ylabel('Tempo [ms]')
    plt.title(f'Central shot (idx={central_shot_idx+1})')
    plt.colorbar(label='Ampiezza')

    fig_path = os.path.join(
        "data", "shots",
        os.path.splitext(os.path.basename(vp_model_path))[0] + "_THEIR_PARAMETERS3.png"
    )
    plt.savefig(fig_path, dpi=200, bbox_inches='tight')
    plt.close()

    print(f"Central shot figure saved at: {fig_path}")
